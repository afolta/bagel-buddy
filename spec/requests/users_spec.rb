...
RSpec.describe "Users", type: :request do
  ...
  describe "PATCH /users/:id with valid parameters" do
    ...
    it 'returns a message that the user was updated' do
      expect(@response_message).to include({"message"=>"User address updated successfully"})
    end
  end

  describe "PATCH /users/:id with invalid parameters" do
    before do
      patch "/users/#{user.id}", params: { address: "NOT A REAL ADDRESS" }
      @response = response
      @response_message = JSON.parse(@response.body)
    end

    it 'returns an HTTP status of of Unprocessable Entity' do
      expect(@response).to have_http_status(422)
    end

    it 'returns a message that the address is invalid' do
      expect(@response_message).to include({"message"=>"Invalid Address"})
    end
  end
end
